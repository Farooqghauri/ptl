...
def load_databases():
    global statutes_db, judgments_db
    # Load Statutes
    if statutes_db is None and os.path.exists(STATUTES_PATH):
        try:
            ...
            statutes_db = pd.DataFrame()

    # Load Judgments
    if judgments_db is None and os.path.exists(JUDGMENTS_PATH):
        try:
...
def smart_search(df, query):
    ...
    # 2. STRICT NUMBER RULE (The Fix)
    # If the user types a number (e.g., "302"), we ONLY show rows that contain that number.
...
async def legal_search(query: str):
    ...
    for _, row in matches.iterrows():
        context_results.append(f"⚖️ STATUTE: {row['statute']} Section {row['section']} - {row['text']}")

    # --- LAYER 2: JUDGMENTS ---
    matches = smart_search(judgments_db, query)
    for _, row in matches.iterrows():
        summary_snippet = row['summary'][:300].replace("\n", " ")
        context_results.append(f"🏛️ PRECEDENT: {row['title']} ({row['citation']})\nRuling: {summary_snippet}...")
