...
function createEmptySession(name?: string): Session {
  ...
  const now = nowIso();
  // Put a welcome assistant message, not a "system" role, to avoid type mismatch
  ...
  return {
    ...
    messages: [welcome], // keep assistant welcome in messages (type matches storage)
    createdAt: now,
    updatedAt: now,
  } as Session;
}
...
export default function PTLAIDraftingSearch(): React.ReactElement {
  // NOTE: removed unused `sessions` state to satisfy eslint/no-unused-vars
  ...
  const exportCurrentSessionAsTxt = (): void => {
    ...
    (currentSession.messages ?? []).forEach((msg) => {
      const who = msg.role === "user" ? "USER" : "ASSISTANT"; // no "system"
    ...
    <div className="flex flex-col h-full min-h-screen bg-gray-50">
      <header className="flex items-center justify-between p-4 bg-white shadow-md">
        ...
        <div className="flex gap-2">
          ...
          <Button type="button" onClick={exportCurrentSessionAsTxt} disabled={!currentSession.messages?.length}>
            Export TXT
          </Button>
      ...
      <main className="flex-1 flex flex-col">
        <div ref={chatRef} className="flex-1 p-4 overflow-y-auto bg-white rounded-b-lg shadow-inner mx-4 mt-4 min-h-[320px]">
        ...
        <form onSubmit={handleSubmit} className="flex p-3 border-t bg-white shadow-sm gap-2 items-center mx-4">
          <input
            type="text"
            placeholder={lastIntent === "drafting" ? "Draft: e.g., 'Draft a plaint under Section 9 CPC'..." : "Search: e.g., 'Most used sections in Pakistan courts'..."}
            value={input}
